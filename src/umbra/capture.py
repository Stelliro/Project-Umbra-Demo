...
the takes averages out room/mic noise, and the individual takes become training
...
def average_recordings(paths, target_shape=(256, 256, 3)) -> np.ndarray:
    ...
    Multi-capture averaging: independent room/mic noise averages toward zero
    across takes, leaving the shared underlying signal.
    """
    paths = list(paths)
    if not paths:
